...
func TestMakeTreeSuccess(t *testing.T) {
	for _, tt := range successTestCases {
		...
		if err != nil {
			var _ error = err
			t.Fatalf("Fail: Build for test case %q returned error %q. Error not expected.",
				tt.name, err)
		}
		if !reflect.DeepEqual(actual, tt.expected) {
			t.Fatalf("Fail: Build for test case %q returned %s but was expected to return %s.",
				tt.name, actual, tt.expected)
		} else {
			t.Logf("Pass(%q): %s", tt.name, actual)
		}
	}
}
